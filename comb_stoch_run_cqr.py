...
for i in range(model_dim):
	for j in range(i+1, model_dim):
		comb_pairs.append((i,j))
print('Combination pairs = ', comb_pairs)

comb_idx = 0 # dim =(model_dim-1)+...+1
prop_idxs = comb_pairs[comb_idx]
...
idx_str1 = f'CQR_#{prop_idxs[0]}_Dropout{drop_out_rate}_multiout_opt=_{n_hidden}hidden_{n_epochs}epochs_{nb_quantiles}quantiles_3layers_alpha{alpha}_lr{lr}'
idx_str2 = f'CQR_#{prop_idxs[1]}_Dropout{drop_out_rate}_multiout_opt=_{n_hidden}hidden_{n_epochs}epochs_{nb_quantiles}quantiles_3layers_alpha{alpha}_lr{lr}'
...
# Train the QR
qr1 = TrainQR(model_name, dataset, idx = idx_str1, cal_hist_size  = cal_hist_size, test_hist_size = test_hist_size, quantiles = quantiles, opt = opt, n_hidden = n_hidden, xavier_flag = xavier_flag, scheduler_flag = scheduler_flag, drop_out_rate = drop_out_rate)
qr1.load_model(n_epochs)
qr2 = TrainQR(model_name, dataset, idx = idx_str2, cal_hist_size  = cal_hist_size, test_hist_size = test_hist_size, quantiles = quantiles, opt = opt, n_hidden = n_hidden, xavier_flag = xavier_flag, scheduler_flag = scheduler_flag, drop_out_rate = drop_out_rate)
qr2.load_model(n_epochs)
